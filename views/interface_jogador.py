...
class InterfaceJogador(ttk.Frame):
    ...
    def receive_move(self, posicao: int, jogador: int,
                     estado_tabuleiro: List[int], armazens: List[int]) -> None:
    ...
    def _desenhar_tabuleiro(self):
        ...
        for i in range(6):
            ...
            self._desenhar_casa(x, y_topo, self.estado_tabuleiro[i], self.cores['sementes_jogador1'])
            self.coords_casas.append((x, y_topo))
        for i in range(6, 12):
            x = x_inicio + (i-6) * self.espaco_centro_casas
            self._desenhar_casa(x, y_base, self.estado_tabuleiro[i], self.cores['sementes_jogador2'])
        ...
        cx = (x1 + x2)/2
        self.canvas.create_text(cx, (y_topo + y_base)/2,text=str(len(self.armazens[0])), font=("Helvetica", 16, "bold"),fill=self.cores['texto_botoes'])
    ...
    def _desenhar_casa(self, x: float, y: float, donos: List[int], _dummy):
        ...
        for i, dono in enumerate(donos):
            ...
            cor = (self.cores['sementes_jogador1']
                if dono == 1 else
                self.cores['sementes_jogador2'])
            self.canvas.create_oval(
                rx-raio, ry-raio, rx+raio, ry+raio,
                fill=cor
            )
